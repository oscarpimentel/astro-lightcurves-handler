...
def search_over_sigma_samples(lcset, b: str, dist_mean, dist_sigma, sigma_m,
    ...
    for lcobj_name in lcset.get_lcobj_names():
        blcobj = lcset[lcobj_name].get_b(b)
        sigmas = blcobj.obse
        ...
        if deleted_points > 0:
            print(f'{deleted_points} obs removed from lcobj_name={lcobj_name} (b={b}; days={blcobj.days[~valid_indexs]})')
...
class LCSet():
    ...
    def get_mean_length_df_bdict(self,
        ...
        for kb, b in enumerate(self.band_names):
            ...
            key = id(self) if index is None else index
            df = pd.DataFrame.from_dict({key: info_dict}, orient='index')
